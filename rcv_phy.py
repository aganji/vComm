...
def decodeFecFunction():
    ...
    if len(manchesterOutput)%FEC_FACTOR != 0:
        ...
        # print binString
        print 'Length of binary string {}'.format(len(binString))
        if binString[0:8] == '00000000' and binString[len(binString)-8:len(binString)] == '11111111':
            sendDataOverSocket(binString[8:len(binString)-8])
        else:
...
actualSize = packetByteSize + 1

str1="FAIL"
failStr=str1.ljust(actualSize)
failBin=toBinary(failStr)
failStrAscii = fromBinary(failBin)
